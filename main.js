(function () {
  ...
  function setupRipples() {
    ...
    function drop(x, y, strength) {
      const amp = 0.136 + 0.183 * strength, r = 1.6 + 2.2 * strength;
      press(x, y, amp, r, 0);
    ...
    document.addEventListener("pointermove", (e) => {
      ...
      if (lastMove) {
        ...
        if (dist > 2 && inCard) {
          const amp = 0.0153 + Math.min(speed, 2) * 0.0165;
    ...
    const schedule = () => {
      ...
      ambient = setTimeout(() => {
        if (!document.hidden) drop(W * (0.1 + Math.random() * 0.8), H * (0.1 + Math.random() * 0.8), 0.118 + Math.random() * 0.177);
        schedule();
